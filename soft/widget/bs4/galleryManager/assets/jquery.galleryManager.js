(function ($) {
    ...
    function galleryManager(el, options) {
        ...
        function createEditorElement(id, src, name, description) {
            ...
            var html = '<div class="photo-editor row">' +
                ...
                (opts.hasName
                    ...
                    '<label class="control-label" for="photo_name_' + id + '">' + galleryDefaults.nameLabel + ':</label>' +
                ...
                (opts.hasDesc
                    ?
                    '<div class="form-group">' +
                    '<label class="control-label" for="photo_description_' + id + '">' + galleryDefaults.descriptionLabel + ':</label>' +
                    '<input class="form-control" name="photo[' + id + '][description]" rows="3" cols="40" class="input-xlarge" id="photo_description_' + id + '">' + htmlEscape(description) + '</input>' +
                    '</div>' : '') +
        ...
        if (opts.hasName || opts.hasDesc) {
            photoTemplate += '<span class="editPhoto btn btn-primary btn-xs"><i class="fa fa-pen"></i></span> ';
        }

        photoTemplate += '<span class="deletePhoto btn btn-danger btn-xs"><i class="fa fa-trash"></i></span>' +
        ...
        $('.save-changes', $editorModal).click(function (e) {
            e.preventDefault();
            $.post(opts.updateUrl, $('input, input', $editorForm).serialize() + csrfParams, function (data) {
